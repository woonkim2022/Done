...
class PlanTextVC: UIViewController {
    ...
    @IBAction func planInputBtn(_ sender: Any) {
        ...
        if planTextState == true {
            ...
            self.planTextField.text = ""  //입력 완료 후 텍스트필드 text 빈배열되도록 하기
            
            if planEditState == true {  // 📌 수정상태가 true일 때,
                // 테이블뷰에 입력한 paln text 실시간 갱신되도록 DispatchQueue 설정
    ...
    func style() {
        ...
        planTextField.borderStyle = .none // 텍스트필드 테두리 제거
        planTextField.layer.sublayerTransform = CATransform3DMakeTranslation(10, 0, 0); // 텍스트필드 left padding
        planTextField.placeholder =  "플랜을 입력해보세요.(10자 이하)"
